...
public class InputProcessorSystem implements GameSystem {
  ...
  public void update(World world, long deltaTime) {
    world.getEntitiesWithComponents(List.of(Input.class, Velocity.class)).forEach(entity -> {
      ...
      newDx = switch (input.getCurrentHInput()) {
      ...
      if (isClimbing) {
        newDy = switch (input.getCurrentVInput()) {
